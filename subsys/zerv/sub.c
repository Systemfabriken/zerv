...
int sub_notify(struct sub *sub, struct pub *emittor, const void *_data, size_t _data_size)
{
	if (sub == NULL || _data == NULL || _data_size == 0) {
		LOG_DBG("Invalid arguments: sub: %p, data: %p, data_size: %d", sub, _data,
			_data_size);
		return -EINVAL;
	}
	...
	LOG_DBG("%s heap diff after alloc: %i heap usage: %d bytes of %d total -> %d %%", sub->name,
		...
		post_memstat.allocated_bytes, sub->heap->heap.init_bytes,
...
int sub_wait(struct sub *sub, k_timeout_t timeout)
{
	if (sub == NULL) {
		...
		LOG_DBG("Received notification from publisher %s @ %p to subscriber %s @ %p with "
			"size %d",
			sub->last_notification->emittor->name, sub->last_notification->emittor,
			sub->name, sub, sub->last_notification->size);
...
void sub_free_internal(struct sub *_sub)
{
	if (_sub == NULL) {
		...
		LOG_DBG("%s heap diff after free: %i heap usage: %d bytes of %d total -> %d %%",
			_sub->name, (int)post_memstat.free_bytes - (int)memstat.free_bytes,
